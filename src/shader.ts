...
export function createRendererShader<L extends TgpuBindGroupLayout>(
  layout: L,
  w: number,
  h: number,
) {
  return tgpu
    .computeFn({
      ...
      const p = layout.$.params as any;
      const ot = layout.$.outputTex as any;

      if (id.x >= w || id.y >= h) {
        return;
      }

      const cx = p.centerX + (d.f32(id.x) - d.f32(w) / 2.0) * p.scale;
      const cy = p.centerY + (d.f32(id.y) - d.f32(h) / 2.0) * p.scale;
      ...
      const t = d.f32(iter) / d.f32(p.maxIterations);
      ...
      const color = std.select(
        ...
        iter === p.maxIterations,
      );
      std.textureStore(ot, d.vec2u(id.x, id.y), color);
    })
    .$uses({ w, h, layout });
}
